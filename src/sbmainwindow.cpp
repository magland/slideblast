...
SBMainWindow::SBMainWindow() {
    ...
    d->add_parameter("build_movie_command","Build movie command","ffmpeg -framerate $FPS$ -pattern_type glob -i '$WORKDIR$/resized*.jpg' -i $AUDIO_FILE$ -c:v libx264 -strict -2 $OUTPUT_FILE$");
